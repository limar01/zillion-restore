...
> **v1.4.2 (2026-09-09):** **NEW PROJECT REPO — `limar01/ZillionApp` (Boss's arena.ai project).** Deploy key **`github-za`** (registered 2026-09-09, write access, `ssh -T` = "Hi limar01/ZillionApp!"; pubkey public-safe; keypair on phone `~/.ssh/id_ed25519_za`). Phone clone: `~/zapp` (origin `git@github-za:limar01/ZillionApp.git`). Contents: `ARENA_ENDPOINTS_LIST.md` (full endpoint map), `ARENA_ENDPOINTS_REPORT.md` (analysis: cookie-session auth, SSE chat streams, R2/S3 presigned uploads, server-action votes, recaptcha v3+v2 anti-bot), `recaptcha_solver.py` (v3 = Playwright real browser; v2 = CapSolver/2Captcha; env-only keys). Sandbox working copy: `/home/user/arena_analysis/`. Arena findings: recaptcha v3 ENTERPRISE sitekey `6LeTGMcsAAAAALuIlkVwIxaAuZA8VledA6d3Nnb0` (public in their HTML), actions `sign_up|agentic_chat_submit|chat_retry|pairwise_feedback|pointwise_feedback`; WAF headers `x-api-waf-token` + `x-arena-request-caller: web-ssr-public`. `/tmp` on phone = shell-owned (exec user gets PermissionError) — write to `~/` instead.
...
15. **PROJECT FILES AND COMMANDS LIVE ON THE PHONE:** New project home = `~/Projects/workspace/<PROJECT>/` in internal Termux storage; Boss declares the active project. Existing owner-approved phone project paths (for example `~/zapp`) remain valid and must not be silently moved or duplicated. Project repositories, downloads, dependencies, builds, tests, artifacts, and working logs belong on the phone, not in the cloud sandbox.
